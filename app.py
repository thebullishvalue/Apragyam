"""
Apragyam (अप्रज्ञम) — Inverted Portfolio Intelligence  |  A @thebullishvalue Product
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Contrarian inverse portfolio curation — weakness is strength.
Weakest candidates receive the most weight. All signals inverted.

Architecture:
  regime.py         → MarketRegimeDetector (inverted), compute_conviction_signals (inverted)
  portfolio.py      → compute_conviction_based_weights() (weakness-weighted)
  backdata.py       → generate_historical_data()
  charts.py         → Plotly chart builders
  strategies.py     → 80+ BaseStrategy implementations

Pipeline (2 phases):
  Phase 1: Data fetching + inverted regime detection
  Phase 2: Inverted conviction-based portfolio curation (ALL strategies)
...
VERSION = "v7.3.0"
PRODUCT_NAME = "Apragyam"
COMPANY = "@thebullishvalue"

st.set_page_config(
    page_title="APRAGYAM | Inverted Portfolio Intelligence",
    page_icon="📉",
    layout="wide",
...
def _render_portfolio_tab(portfolio: pd.DataFrame, current_df: pd.DataFrame, capital: float):
    ...
    render_section_header(
        "Inverted Portfolio Holdings",
        f"{len(portfolio)} positions · contrarian weakness-weighted curation",
        icon="briefcase",
        accent="amber",
    )
    ...
    if CHARTS_AVAILABLE and not portfolio_with_signals.empty:
        ...
        st.caption(
            "INVERTED: High score = weakest stock · Low score = strongest stock · RSI (25%) · Osc (25%) · WT (20%) · Z (15%) · MA (15%)"
        )
    elif not portfolio_with_signals.empty:
        conv_cols = [c for c in ["symbol", "rsi_value", "osc_value", "zscore_value", "ma_count", "wt1_value", "conviction_score"]
    ...
    st.download_button(
        ...
        data=buf.getvalue(),
        file_name=f"apragyam_portfolio_{datetime.now().strftime('%Y%m%d')}.csv",
        mime="text/csv",
        width='stretch',
...
def _render_position_guide_tab(portfolio: pd.DataFrame, current_df: pd.DataFrame):
    ...
    deep_value = sum(1 for s in scores if s >= 65)
    value = sum(1 for s in scores if 50 <= s < 65)
    hold = sum(1 for s in scores if 35 <= s < 50)
    too_strong = sum(1 for s in scores if s < 35)

    with c1:
        render_metric_card("Deep Value", str(deep_value), "Weakest candidates (≥65)", "danger")
    with c2:
        render_metric_card("Value", str(value), "Weak candidates (50-64)", "warning")
    with c3:
        render_metric_card("Hold", str(hold), "Neutral (35-49)", "info")
    with c4:
        render_metric_card("Too Strong", str(too_strong), "Avoid — too strong (<35)", "success")
    ...
    # Tier colors mapping — INVERTED: weakness is valued (red/warm), strength is avoided (green/cool)
    tier_colors = {
        'Deep Value': ('#FB7185', '#FDA4AF', 'rgba(251, 113, 133, 0.15)', 'rgba(251, 113, 133, 0.3)'),
        'Value': ('#D4A853', '#E8C478', 'rgba(212, 168, 83, 0.15)', 'rgba(212, 168, 83, 0.3)'),
        'Hold': ('#6EE7B7', '#A7F3D0', 'rgba(52, 211, 153, 0.1)', 'rgba(52, 211, 153, 0.2)'),
        'Too Strong': ('#34D399', '#6EE7B7', 'rgba(52, 211, 153, 0.15)', 'rgba(52, 211, 153, 0.3)'),
    }
    ...
    for _, row in sorted_df.iterrows():
        ...
        wt1_val = row.get("wt1_value")
        price = row.get("price", 0)
        weight = row.get("weightage_pct", 0)

        # Determine tier — INVERTED: high score = weakest = most desired
        if score >= 65:
            tier_name = "Deep Value"
            emoji = "🔴"
        elif score >= 50:
            tier_name = "Value"
            emoji = "🟠"
        ...
        wt_str = f"{wt1_val:.1f}" if wt1_val is not None and not pd.isna(wt1_val) else "—"
...
def _render_regime_tab(regime_result: Dict, regime_series: List, training_data: List = None):
    ...
    with col_factors:
        st.markdown("**Factor Scores** (INVERTED: −2 = strong/bullish → +2 = weak/bearish):")
        factor_order = [
        ...
        for fkey, fname, label_key in factor_order:
            ...
            bar_color = "#ef4444" if fs >= 0.5 else ("#10b981" if fs <= -0.5 else "#f59e0b")
            st.markdown(f"""
    ...
    if regime_series_to_use and len(regime_series_to_use) > 0:
        ...
        def regime_color(regime: str) -> str:
            """INVERTED: bull regimes (triggered by real weakness) = danger, bear (triggered by real strength) = success."""
            r = regime.upper().replace("-", "_")
            if r in ("STRONG_BULL", "BULL", "WEAK_BULL"):
                return "danger"    # rose — triggered by real-world weakness
            elif r in ("BEAR", "CRISIS"):
                return "success"   # emerald — triggered by real-world strength
            elif r == "WEAK_BEAR":
                return "warning"   # amber — transitional
...
def _render_system_tab(training_window: List):
    ...
    details = {
        "Version": VERSION,
        "Curation Method": "INVERTED Conviction-Based (All 80+ Strategies)",
        "Weight Formula": "(inverted_conviction / total) × 100 — weakness → highest weight",
    ...
    with c1:
        render_metric_card(
            "📊 Inverted Conviction Scoring",
            "0-100 Range (Inverted · 5 Pillars)",
            "Signals: RSI (25%) · Oscillator (25%)<br>WaveTrend (20%) · Z-Score (15%) · MA (15%)<br><br>Formula: (-raw + 2) / 4 × 100<br>Bearish=100, Bullish=0",
            "danger"
        )

    with c2:
        render_metric_card(
            "⚖️ Weakness Weighting",
            "(inv_conviction / total) × 100",
            "Bounds: Min 1% · Max 10%<br>Selection: Top 30 by weakness<br><br>Weakest candidates get most weight",
            "warning"
        )
...
def _render_header() -> None:
    ...
    render_header(
        ...
        tagline="Inverted Contrarian Curation · Weakness Is Strength · All 95 Strategies · Live NSE Data"
    )
...
def _render_landing_page():
    ...
    Adapted for Apragyam's portfolio intelligence features.
    """
    ...
    with col1:
        ...
        <div class='system-card portfolio'>
            ...
            <p>Inverted contrarian curation — weakest candidates receive highest weight via inverted composite scoring.</p>
            <div class='spec'>
                <span>Signals:</span> RSI (25%) + Osc (25%) + WT (20%) + Z (15%) + MA (15%) — ALL INVERTED<br>
                <span>Selection:</span> Top 30 by weakness score<br>
                <span>Weighting:</span> (inverted_conviction / total) × 100<br>
                <span>Dispersion:</span> SIP + Swing modes (weakness-boosted)
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class='system-card regime'>
            ...
            <p>Inverted seven-factor regime detection — celebrates weakness, fears strength. All factor scores negated.</p>
            <div class='spec'>
                <span>Regimes:</span> Inverted: Bull=Crisis, Crisis=Opportunity<br>
                <span>Factors:</span> All 7 factors score-negated<br>
                <span>Output:</span> Inverted confidence + contrarian mix<br>
    ...
    with col3:
        ...
        <div class='system-card strategies'>
            ...
            <p>Ninety-five strategies inverted — outputs fed through contrarian lens. Weakness signals amplified, strength penalised.</p>
            <div class='spec'>
                <span>Categories:</span> Momentum + Reversal + Breakout (all inverted)<br>
                <span>Universe:</span> Nifty 500 + F&O symbols<br>
                <span>Style:</span> SIP + Swing (weakness-concentrated)<br>
                <span>Strategies:</span> 95 engines, contrarian overlay
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div class='landing-prompt'>
        ...
        <p>Configure via the <strong>Sidebar</strong>: select <strong>Analysis Date</strong>, <strong>Investment Style</strong>, <strong>Capital</strong>, and <strong>Number of Positions</strong>.<br>
           Execute <strong>Run Analysis</strong> to run all 95 strategies through the <strong>inverted contrarian lens</strong>.<br>
           <span style="color:var(--ink-secondary); font-size:0.85em; margin-top:0.5rem; display:inline-block;">System will invert regime signals · Score weakness · Weight the weakest candidates heaviest</span></p>
    </div>
    """, unsafe_allow_html=True)
...
def _render_results(capital: float):
    ...
    <div class="app-footer">
        <div class="content">
            © {now_ist.year} <strong>Apragyam</strong> &nbsp;·&nbsp; @thebullishvalue &nbsp;·&nbsp; v{VERSION} &nbsp;·&nbsp; {now_ist.strftime("%Y-%m-%d %H:%M:%S IST")}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def _run_analysis(
    ...
    try:
        ...
        log.main_header(f"APRAGYAM | Inverted Portfolio Intelligence | {VERSION}", run_details)
        ...
        try:
            ...
            progress_bar(progress_container, 50, "Computing inverted conviction", f"{len(aggregated_holdings)} candidates · weakness-weighted")
            ...
            progress_bar(progress_container, 85, "Inverted portfolio curated", f"{len(st.session_state.portfolio)} weakness-weighted positions")
...
def _render_footer() -> None:
    ...
    st.markdown(
        ...
        f'<div class="content">'
        f'© {ist_now.year} <strong>{PRODUCT_NAME}</strong> &nbsp;·&nbsp; {COMPANY} &nbsp;·&nbsp; v{VERSION} &nbsp;·&nbsp; INVERTED &nbsp;·&nbsp; {ist_now.strftime("%Y-%m-%d %H:%M:%S IST")}'
        f'</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
...
def main():
    ...
    with st.sidebar:
        ...
        <div style="text-align:center;padding:0.75rem 0 1rem 0;">
            <div style="font-family:var(--display);font-size:1.5rem;font-weight:700;color:var(--amber);letter-spacing:0.06em;">APRAGYAM</div>
            <div style="font-family:var(--data);color:var(--ink-tertiary);font-size:0.65rem;margin-top:0.2rem;letter-spacing:0.08em;text-transform:uppercase;">अप्रज्ञम | Inverted Portfolio Intelligence</div>
        </div>
        """,
            unsafe_allow_html=True,
        )
